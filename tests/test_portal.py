"""Tests for POST /portal-session (Stripe Billing Portal)."""
...
def _install_portal_stub(monkeypatch, *, portal_session=None, portal_raise=None):
...
def test_portal_session_success(portal_client, monkeypatch):
    rec = _make_customer()
    _install_portal_stub(monkeypatch)
...
def test_portal_session_stripe_failure_returns_502(portal_client, monkeypatch):
    ...
    _install_portal_stub(monkeypatch, portal_raise=RuntimeError("nope"))
    monkeypatch.setenv("STRIPE_SECRET_KEY", "sk_test_x")
    monkeypatch.setenv("STRIPE_PORTAL_RETURN_URL", "https://example.com/billing")
